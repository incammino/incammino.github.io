...
async function fromAPI(){
  ...
  // Fallback: alcuni giorni il testo finisce in comment_t
  var commentText  = (res[4]&&res[4].length>30) ? res[4] : (res[2]&&res[2].length>30) ? res[2] : '';
  var commentTitle = (res[4]&&res[4].length>30) ? res[2] : '';
  return {reference:res[0], text:res[1], commentTitle:commentTitle, commentAuthor:res[3], commentText:commentText};
}
...
function mergeDefaultsIntoPrayers(userPrayers) {
  /* Mappa id → preghiera di default per lookup rapido */
  var defaultMap = {};
  DEFAULT_PRAYERS.forEach(function(dp){ defaultMap[dp.id] = dp; });

  /* 1. Aggiorna il contenuto delle preghiere di default già presenti
        (testo, emoji, tag, nome) senza spostarne la posizione.
        Le preghiere personali (isDefault falsy) restano intatte. */
  var updated = userPrayers.map(function(p){
    if(p.isDefault && defaultMap[p.id]) return Object.assign({}, defaultMap[p.id]);
    return p;
  });

  /* 2. Aggiunge in testa le preghiere di default completamente nuove */
  var userIds = updated.map(function(p){ return p.id; });
  var newDefaults = DEFAULT_PRAYERS.filter(function(dp){
    return userIds.indexOf(dp.id) === -1;
  });
  if(!newDefaults.length) return updated;
  return newDefaults.concat(updated);
}
